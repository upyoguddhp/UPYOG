...
//       }
		try {
			log.info("add det : " + tradeLicenseRequest.getLicenses().get(0).getTradeLicenseDetail().getAddress()
					.getAdditionalDetail());
			log.info("add det : "
					+ tradeLicenseRequest.getLicenses().get(0).getTradeLicenseDetail().getAddress().getAddressLine1());
		} catch (Exception e) {
	...
	private void enrichPreCreateNewTLValues(TradeLicenseRequest tradeLicenseRequest) {
		tradeLicenseRequest.getLicenses().forEach(license -> {
			if (StringUtils.equals(license.getBusinessService(), businessService_NewTL)) {
				enrichmentService.enrichCreateNewTLValues(tradeLicenseRequest.getRequestInfo().getUserInfo().getUuid(),
						license);
			}
		});
	...
	public List<TradeLicense> search(TradeLicenseSearchCriteria criteria, RequestInfo requestInfo,
		...
		List<String> rolesWithinTenant = getRolesWithinTenant(criteria.getTenantId(),
	...
	private List<TradeLicense> filterLicensesBasedOnRolesWithinTenantId(List<TradeLicense> licenses,
		...
		if (StringUtils.equalsIgnoreCase(requestInfo.getUserInfo().getType(), TLConstants.ROLE_CODE_EMPLOYEE)) {
			...
			tempLicenses = tempLicenses.stream()
					.filter(license -> !StringUtils.equalsIgnoreCase(license.getStatus(), TLConstants.STATUS_INITIATED))
...
//        // send notifications
	...
	private void generateDemandAndBill(TradeLicenseRequest tradeLicenseRequest) {
	    ...
	    tradeLicenseRequest.getLicenses().forEach(license -> {
	        ...
	        // Allow both NEW and RENEWAL
	    	if ((StringUtils.equalsIgnoreCase(TLConstants.APPLICATION_TYPE_NEW, license.getApplicationType())
	    	        || StringUtils.equalsIgnoreCase(TLConstants.APPLICATION_TYPE_RENEWAL, license.getApplicationType()))
	    	    && StringUtils.equalsIgnoreCase(TLConstants.ACTION_RETURN_TO_INITIATOR_FOR_PAYMENT,
	    	            license.getAction())){
	            ...
	            savedDemands = demandService.generateDemand(
	            ...
	            if (CollectionUtils.isEmpty(savedDemands)) {
	                throw new CustomException(
	                        "INVALID CONSUMERCODE",
	                        "Bill not generated due to no Demand found for the given consumerCode");
	            }

	            // Generate Bill (IMPORTANT: use same businessService)
	            ...
	            log.info("#### Bill generated for: " + license.getApplicationNumber());
	        }
	    });
	}
	...
	private TradeLicenseRequest enrichPreUpdateNewTLValues(TradeLicenseRequest tradeLicenseRequest,
		...
		TradeLicenseRequest tempTradeLicenseRequest = TradeLicenseRequest.builder()
				.requestInfo(tradeLicenseRequest.getRequestInfo()).licenses(new ArrayList()).build();
		...
		for (int i = 0; i < tradeLicenseRequest.getLicenses().size(); i++) {
			...
			if (null == license.getTradeLicenseDetail()
				...
				TradeLicenseSearchCriteria tradeLicenseSearchCriteria = TradeLicenseSearchCriteria.builder()
						.businessService(businessServicefromPath).tenantId(license.getTenantId())
						.applicationNumber(license.getApplicationNumber()).build();
				List<TradeLicense> licenses = getLicensesWithOwnerInfo(tradeLicenseSearchCriteria,
						tradeLicenseRequest.getRequestInfo());
				...
				if (StringUtils.equalsIgnoreCase(TLConstants.STATUS_APPROVED, licenses.get(0).getStatus())
						&& (StringUtils.equalsIgnoreCase(TLConstants.ACTION_RETURN_TO_INITIATOR, license.getAction())
								|| StringUtils.equalsIgnoreCase(TLConstants.ACTION_RETURN_TO_INITIATOR_FOR_PAYMENT,
										license.getAction())
								|| StringUtils.equalsIgnoreCase(TLConstants.ACTION_FORWARD_TO_VERIFIER,
										license.getAction())
								|| StringUtils.equalsIgnoreCase(TLConstants.ACTION_CLOSE, license.getAction()))) {
...
//		sendMail;
	...
	private void createAndUploadPDF(TradeLicenseRequest tradeLicenseRequest) {
		...
		List<TradeLicense> newList = tradeLicenseRequest.getLicenses();
...
//		}
	...
	public ProcessInstanceResponse updateState(UpdateTLStatusCriteriaRequest updateTLStatusCriteriaRequest) {
		...
		ProcessInstance processInstance = ProcessInstance.builder()
				...
				.businessService(businessService_TL).moduleName("TL")
				...
				.comment(updateTLStatusCriteriaRequest.getUpdateTLStatusCriteria().getComment()).build();
		...
		if (StringUtils.equalsIgnoreCase(updateTLStatusCriteriaRequest.getUpdateTLStatusCriteria().getAction(),
				"VERIFY")) {
			tlRepository.updateTlStatus(updateTLStatusCriteriaRequest.getUpdateTLStatusCriteria().getBusinessId(),
					"VERIFIED");
		} else if (StringUtils.equalsIgnoreCase(updateTLStatusCriteriaRequest.getUpdateTLStatusCriteria().getAction(),
				"APPROVE")) {
			tlRepository.updateTlStatus(updateTLStatusCriteriaRequest.getUpdateTLStatusCriteria().getBusinessId(),
					"APPROVED");
		}
	...
	public ApplicationStatusChangeRequest updateStateOfApplication(
			ApplicationStatusChangeRequest applicationStatusChangeRequest) {

		// search TL
		TradeLicenseSearchCriteria criteria = TradeLicenseSearchCriteria.builder()
				.tenantId(applicationStatusChangeRequest.getTenantId()).businessService(businessService_TL)
				.applicationNumber(applicationStatusChangeRequest.getApplicationNumber()).build();
		List<TradeLicense> licenses = repository.getLicenses(criteria);
		...
		if (StringUtils.equals(applicationStatusChangeRequest.getApplicationStatus(), STATUS_APPLIED)) {
			if (!(StringUtils.equals(tl.getStatus(), STATUS_INITIATED)
				...
				throw new CustomException("STATUS_CHANGE_NOT_ALLOWED",
						"Currently Status can't be changed to " + STATUS_APPLIED);
			}
		}
	...
	public void processResponse(TradeLicenseResponse response) {
		...
		if (!CollectionUtils.isEmpty(response.getLicenses())) {
			...
			response.setApplicationApproved((int) response.getLicenses().stream()
					.filter(license -> StringUtils.equalsIgnoreCase(STATUS_APPROVED, license.getStatus())).count());
	...
	public void validateTradeLicenseCertificateGeneration(TradeLicense tradeLicense, RequestInfo requestInfo) {
		...
		if (StringUtils.isEmpty(tradeLicense.getLicenseNumber())
				...
				&& (tradeLicense.getTradeLicenseDetail().getAddress().getAdditionalDetail().get("district") == null
						|| StringUtils.isEmpty(tradeLicense.getTradeLicenseDetail().getAddress().getAdditionalDetail()
								.get("district").asText()))
				...
				&& tradeLicense.getIssuedDate() == null && tradeLicense.getValidTo() == null
				&& (tradeLicense.getTradeLicenseDetail().getAdditionalDetail().get("tradeCategory") == null
						|| StringUtils.isEmpty(tradeLicense.getTradeLicenseDetail().getAdditionalDetail()
								.get("tradeCategory").asText()))
				&& (tradeLicense.getTradeLicenseDetail().getAdditionalDetail().get("applicantName") == null
						|| StringUtils.isEmpty(tradeLicense.getTradeLicenseDetail().getAdditionalDetail()
								.get("applicantName").asText()))
				&& (tradeLicense.getTradeLicenseDetail().getAdditionalDetail().get("applicantMobileNumber") == null
						|| StringUtils.isEmpty(tradeLicense.getTradeLicenseDetail().getAdditionalDetail()
								.get("applicantMobileNumber").asText()))
				&& StringUtils.isEmpty(tradeLicense.getBusinessService()) && null == requestInfo
				&& null == requestInfo.getUserInfo() && null == requestInfo.getUserInfo().getUserName()) {
...
//		MultipartFile multipartFile = convertResourceToMultipartFile(resource, "filename");
		...
		DmsRequest dmsRequest = DmsRequest.builder().userId(requestInfo.getUserInfo().getId().toString())
				.objectId(tradeLicense.getId()).description(TLConstants.ALFRESCO_COMMON_CERTIFICATE_DESCRIPTION)
				.id(TLConstants.ALFRESCO_COMMON_CERTIFICATE_ID).type(TLConstants.ALFRESCO_COMMON_CERTIFICATE_TYPE)
				.objectName(businessService_NewTL).comments(TLConstants.ALFRESCO_TL_CERTIFICATE_COMMENT)
				.status(STATUS_APPROVED).file(resource).servicetype(tradeLicense.getBusinessService())
				.documentType(TLConstants.ALFRESCO_DOCUMENT_TYPE).documentId(TLConstants.ALFRESCO_COMMON_DOCUMENT_ID)
				.build();
	...
	private Map<String, Object> generateDataForTradeLicensePdfCreate(TradeLicense tradeLicense,
		...
		tlObject.put("tradePremisesAddress",
				tradeLicense.getTradeLicenseDetail().getAddress().getAddressLine1().concat(", ")
						.concat(tradeLicense
								.getTradeLicenseDetail().getAddress().getAdditionalDetail().get("district").asText())
						.concat(", ")
						.concat(tradeLicense.getTradeLicenseDetail().getAddress().getAdditionalDetail().get("wardName")
								.asText())
						.concat(", ").concat(tradeLicense.getTradeLicenseDetail().getAddress().getPincode()));// Trade
																												// Premises
																												// Address
		...
		tlObject.put("licenseApplicantName",
				tradeLicense.getTradeLicenseDetail().getAdditionalDetail().get("applicantName").asText());// License
																											// Applicant
																											// Name
		tlObject.put("applicantContactNo",
				tradeLicense.getTradeLicenseDetail().getAdditionalDetail().get("applicantMobileNumber").asText());// Applicant
																													// Contact
																													// No
		tlObject.put("ulbType",
				tradeLicense.getTradeLicenseDetail().getAddress().getAdditionalDetail().get("ulbType").asText());// ulbType
		tlObject.put("ulbName",
				tradeLicense.getTradeLicenseDetail().getAddress().getAdditionalDetail().get("ulbName").asText());// ulbName
		tlObject.put("licenseeMobileNumber",
				!CollectionUtils.isEmpty(tradeLicense.getTradeLicenseDetail().getOwners())
						? tradeLicense.getTradeLicenseDetail().getOwners().get(0).getMobileNumber()
	...
	public TradeLicenseActionResponse getActionsOnApplication(TradeLicenseActionRequest tradeLicenseActionRequest) {
		...
		tradeLicenseActionRequest.getApplicationNumbers().stream().forEach(applicationNumber -> {
			...
			List<String> rolesWithinTenant = getRolesWithinTenant(applicationTenantId,
					tradeLicenseActionRequest.getRequestInfo().getUserInfo().getRoles());
		...
		applicationActionMaps.entrySet().stream().forEach(entry -> {
			nextActionList.add(
					ApplicationDetail.builder().applicationNumber(entry.getKey()).action(entry.getValue()).build());
		});

		TradeLicenseActionResponse tradeLicenseActionResponse = TradeLicenseActionResponse.builder()
				.applicationDetails(nextActionList).build();
...
//		Map<String, Object> totalTaxMap = new HashMap<>();
		...
		tradeLicenseActionRequest.getApplicationNumbers().stream().forEach(applicationNumber -> {
			...
			licenses = enrichmentService.enrichTradeLicenseSearch(licenses, criteria,
					tradeLicenseActionRequest.getRequestInfo());
			TradeLicense license = null != licenses ? licenses.get(0) : null;

			ApplicationDetail applicationDetail = getApplicationBillUserDetail(license,
					tradeLicenseActionRequest.getRequestInfo());
	...
	public ApplicationDetail getApplicationBillUserDetail(
	        TradeLicense license, RequestInfo requestInfo) {
	    ...
	    if (StringUtils.equalsIgnoreCase(
	        ...
	        getApplicationBillUserDetailForNewTL(
	                applicationDetail, license, requestInfo,
	                businessService, scaleOfBusiness,
	                tradeCategory, periodOfLicense, zone
	        );
	    }
	    ...
	    // RENEWAL LICENSE
	        ...
	        getApplicationBillUserDetailForRenewal(
	                applicationDetail, license, requestInfo
	        );
	    }
	...
	private ApplicationDetail getApplicationBillUserDetailForNewTL(ApplicationDetail applicationDetail,
		...
		try {
			...
			scaleOfBusiness = license.getTradeLicenseDetail().getAdditionalDetail().get(TLConstants.SCALE_OF_BUSINESS)
					.asText();
			...
			throw new CustomException("FETCH_LICENSE_FAILED",
					"Failed to fetch values from trade license while calculating tax.");
		}

		// validating values from TL in DB
		if (StringUtils.isEmpty(scaleOfBusiness) || StringUtils.isEmpty(zone) || StringUtils.isEmpty(tradeCategory)) {
...
//				.get(TLConstants.FEE_STRUCTURE);
		...
		for (Object object : feeStructure) {
			...
			} else if (StringUtils.equalsIgnoreCase(map.get(TLConstants.STRUCTURE_OF).toString(),
					TLConstants.TRADE_CATEGORY)
...
//		Scale Of Business( <b>Small</b>) and License Period ( <b>1 Year </b> ) : <b>15.0</b> <br /> + [Zone (<b>zone1</b>): <b>50.0<b><br /> * Category (<b>Agriculture</b>): <b>0.0</b> ]
		...
		try {
			...
			userDetails.put("Address",
					new String(license.getTradeLicenseDetail().getAddress().getAddressLine1().concat(", "))
							.concat(license.getTradeLicenseDetail().getAddress().getAdditionalDetail().get("wardName")
									.asText().concat(", "))
							.concat(license.getTradeLicenseDetail().getAddress().getAdditionalDetail().get("ulbName")
									.asText().concat(", "))
							.concat(license.getTradeLicenseDetail().getAddress().getAdditionalDetail().get("district")
									.asText().concat(", "))
	...
	private ApplicationDetail getApplicationBillUserDetailForRenewal(ApplicationDetail applicationDetail,
			TradeLicense license, RequestInfo requestInfo) {

		getApplicationBillUserDetailForNewTL(applicationDetail, license, requestInfo, null, null, null, 0, null);
		return applicationDetail;
	}
	public TradeLicenseActionResponse getCountOfAllApplicationTypes(
		...
		try {
			if (null != tradeLicenseActionRequest) {
				statusList = tlRepository.getTypesOfAllApplications(tradeLicenseActionRequest.getIsHistoryCall(),
