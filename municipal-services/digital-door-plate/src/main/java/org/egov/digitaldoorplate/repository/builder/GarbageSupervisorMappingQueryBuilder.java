...
@Component
public class GarbageSupervisorMappingQueryBuilder {

	public static final String CREATE_QUERY = "INSERT INTO eg_ddp_garbage_supervisor_mapping "
			+ "(uuid, tenant_id, supervisor_uuid, contractor_uuid, supervisor_user_uuid, ward_number, "
			+ "is_active, createdby, createddate, lastmodifiedby, lastmodifieddate) "
			+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	public static final String DEACTIVATE_ALL_QUERY = "UPDATE eg_ddp_garbage_supervisor_mapping "
			+ "SET is_active = false, lastmodifiedby = ?, lastmodifieddate = ? "
			+ "WHERE supervisor_uuid = ? AND is_active = true";

	private static final String SEARCH_QUERY = "SELECT * FROM eg_ddp_garbage_supervisor_mapping WHERE 1=1 ";

	public String getSearchQuery(SearchCriteriaGarbageSupervisorMapping criteria,
		...
		if (!CollectionUtils.isEmpty(criteria.getSupervisorUuid())) {
			query.append(" AND supervisor_uuid IN (")
					.append(getPlaceholders(criteria.getSupervisorUuid().size())).append(")");
			preparedStatementValues.addAll(criteria.getSupervisorUuid());
		}
