...
public class Fund extends Auditable {
	...
	/**
	 * code is a unique number given to each fund . ULB may refer this for the short
	 * name
	 */
	@Length(max = 50, min = 2)
	@NotNull
	...
	/**
	 * identifier appears as prefix in all the vouchers accounted in the books of
	 * the Fund. Each fund must have an identifier – each voucher belonging to a
	 * fund must have the identifier embedded in the voucher number for easy
	 * identification. Fund is taken at a voucher head level for each voucher
	 * transaction.
	 */
	@NotNull
	...
	/**
	 * parent adding a parent will create the fund as a sub-fund (child) of a fund
	 * already created (parent fund).
	 * 
	 */
	protected Fund parent;
	/**
	 * active is a boolean value which says whether fund is in use or not . If Fund
	 * is active, then accounting of transactions under the fund is enabled. If Fund
	 * becomes inactive, and no transactions can be accounted under the Fund.
	 */
	@NotNull
	protected Boolean active;

	/**
	 * isParent is updated internally so that system can identify whether the fund
	 * is parent or child. Only child which is not parent for any other fund can
	 * only participate in transaction .
	 */
